...
static void CrashRecoverySignalHandler(int Signal) {
  ...
  //   // signal on a thread we didn't expect it on, the application got a
  //   signal
  //   // outside of a crash recovery context, or something else went horribly
  //   // wrong.
  //   //
  //   // Disable crash recovery and raise the signal again. The assumption here
  //   is
  //   // that the enclosing application will terminate soon, and we won't want
  //   to
  //   // attempt crash recovery again.
  //   //
  //   // This call of Disable isn't thread safe, but it doesn't actually
  //   matter. CrashRecoveryContext::Disable(); raise(Signal);
  ...
  // // Return the same error code as if the program crashed, as mentioned in
  // the
  ...
  // clang/lib/Driver/Driver.cpp) if (Signal == SIGPIPE)
  //   RetCode = EX_IOERR;

  // if (CRCI)
  //   const_cast<CrashRecoveryContextImpl *>(CRCI)->HandleCrash(RetCode,
  //   Signal);
}
